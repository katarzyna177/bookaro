...
class AbandonedOrdersJobTest {
    ...
    private Long placeOrder(Long bookId, int copies){
        ...
        PlaceOrderUseCase.PlaceOrderCommand command = PlaceOrderUseCase.PlaceOrderCommand
                .builder()
                .recipient(recipient())
                .item(new PlaceOrderUseCase.OrderItemCommand(bookId, copies))
                //.item(new OrderItemCommand(jcip.getId(), 10))
                .build();
        //PlaceOrderResponse response = service.placeOrder(command);
        return placeOrderService.placeOrder(command).getRight();
    }

    private Recipient recipient(){
        return Recipient.builder().email("marek@example.pl").build();
    }
